#!/bin/bash
# LE31 daily research fetcher for 2026-08-15, 7d window 2026-08-09..2026-08-15
# START=1786233600  END=1786838399
# Adapted from /opt/data/le31_mmm3_research_work/fetch_le31_2026-08-14.sh
set -u

UA='LE31Research/1.0 (research; contact: hermes-agent@nousresearch.local)'
RAW=/tmp/le31-daily-2026-08-15
START=1786233600
END=1786838399
INDEX=/tmp/le31-daily-2026-08-15/index.json.tmp
LOG=/tmp/le31-daily-2026-08-15/_fetch.log
...
log_line "==== LE31 daily fetch - window 2026-08-09..2026-08-15 (epoch $START..$END) ===="
...
echo "[3] OpenAlex (3 queries, date-filtered 2026-08-09..2026-08-15)"
for pair in \
  "restaurant_kitchen_inventory|restaurant kitchen inventory" \
  "kitchen_display_system|kitchen display system" \
  "telegram_bot_restaurant|telegram bot restaurant"; do
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.openalex.org/works?search=$(printf '%s' "$q" | jq -sRr @uri)&per-page=20&filter=from_publication_date:2026-08-09,to_publication_date:2026-08-15"
  fetch "openalex" "$url" "openalex_${slug}.json"
done

echo "[4] GitHub Search Repositories (7 queries, created:2026-08-09..2026-08-15)"
for pair in \
  ...
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.github.com/search/repositories?q=$(printf '%s' "$q" | jq -sRr @uri)+created:2026-08-09..2026-08-15&sort=updated&order=desc&per_page=30"
...
ENVELOPE=/tmp/le31-daily-2026-08-15/index.json
