#!/bin/bash
# LE31 daily research fetcher — 2026-08-09
# Mirrors fetch_le31.sh patterns; uses spec-given literal epoch + date strings.
# NOTE: spec epoch 1754265600..1754784000 resolves to 2025-08-04..2025-08-10
# (prose-vs-epoch year-disagree). Hard-rule = use literal binding contract.
set -u

UA='LE31Research/1.0 (research; contact: hermes-agent@nousresearch.local)'
RAW=/tmp/le31-daily-2026-08-09
START=1754265600
END=1754784000
INDEX=/tmp/le31-daily-2026-08-09/index.json.tmp
> "$INDEX"
...
echo "[1] HN Algolia (6 queries) — window $START..$END"
for pair in \
...
echo "[3] OpenAlex (3 queries) — spec date strings as written"
for pair in \
  "restaurant_kitchen_inventory|restaurant kitchen inventory" \
  "kitchen_display_system|kitchen display system" \
  "telegram_bot_restaurant|telegram bot restaurant"; do
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.openalex.org/works?search=$(printf '%s' "$q" | jq -sRr @uri)&filter=from_publication_date:2026-08-03,to_publication_date:2026-08-09&per_page=20"
  fetch "openalex" "$url" "openalex_${slug}.json"
done

echo "[4] GitHub Search (7 queries) — spec pushed:>2026-08-02"
for pair in \
  "restaurant_pos_python|restaurant POS language:python pushed:>2026-08-02" \
  "aiogram_restaurant|aiogram bot restaurant pushed:>2026-08-02" \
  "kitchen_display|kitchen display pushed:>2026-08-02" \
  "fastapi_sse|fastapi SSE pushed:>2026-08-02" \
  "restaurant_inventory|restaurant inventory pushed:>2026-08-02" \
  "telegram_kitchen|telegram kitchen pushed:>2026-08-02" \
  "pos_mcp|POS MCP pushed:>2026-08-02"; do
  slug=${pair%%|*}; q=${pair##*|}
...
echo "[10] ScienceDirect (blocked, no fetch)"
blocked_entry='{"filename":"sciencedirect_blocked.json","source":"sciencedirect","url":"https://www.sciencedirect.com/","http_status":"BLOCKED","byte_size":0,"url_effective":"https://www.sciencedirect.com/","meta":{"note":"blocked on VPS — Cloudflare, do not retry"}}'
echo "$blocked_entry," >> "$INDEX"

sed -i '$ s/,$//' "$INDEX"
echo ']' >> "$INDEX"
mv "$INDEX" /tmp/le31-daily-2026-08-09/index.json

echo
echo "=== File count ==="
ls -1 "$RAW" | grep -v 'index.json\|fetch_le31.sh' | wc -l
