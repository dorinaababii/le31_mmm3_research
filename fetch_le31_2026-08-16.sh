#!/bin/bash
# LE31 daily research fetcher for 2026-08-16, 7d window 2026-08-10..2026-08-16
# START=1786320000  END=1786924799
# Adapted from /opt/data/le31_mmm3_research_work/fetch_le31_2026-08-15.sh
set -u

UA='LE31Research/1.0 (research; contact: hermes-agent@nousresearch.local)'
RAW=/tmp/le31-daily-2026-08-16
START=1786320000
END=1786924799
INDEX=/tmp/le31-daily-2026-08-16/index.json.tmp
LOG=/tmp/le31-daily-2026-08-16/_fetch.log
...
log_line "==== LE31 daily fetch - window 2026-08-10..2026-08-16 (epoch $START..$END) ===="
...
echo "[3] OpenAlex (3 queries, date-filtered 2026-08-10..2026-08-16)"
for pair in \
  "restaurant_kitchen_inventory|restaurant kitchen inventory" \
  "kitchen_display_system|kitchen display system" \
  "telegram_bot_restaurant|telegram bot restaurant"; do
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.openalex.org/works?search=$(printf '%s' "$q" | jq -sRr @uri)&per-page=20&filter=from_publication_date:2026-08-10,to_publication_date:2026-08-16"
  fetch "openalex" "$url" "openalex_${slug}.json"
done

echo "[4] GitHub Search Repositories (7 queries, created:2026-08-10..2026-08-16)"
for pair in \
  ...
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.github.com/search/repositories?q=$(printf '%s' "$q" | jq -sRr @uri)+created:2026-08-10..2026-08-16&sort=updated&order=desc&per_page=30"
...
ENVELOPE=/tmp/le31-daily-2026-08-16/index.json
