#!/bin/bash
# LE31 daily research fetcher for 2026-08-17, 7d window 2026-08-11..2026-08-17
# START=1786924800  END=1787529599
set -u

UA='LE31Research/1.0 (research; contact: hermes-agent@nousresearch.local)'
RAW=/tmp/le31-daily-2026-08-17
START=1786924800
END=1787529599
INDEX=/tmp/le31-daily-2026-08-17/index.json.tmp
LOG=/tmp/le31-daily-2026-08-17/_fetch.log
...
log_line "==== LE31 daily fetch - window 2026-08-11..2026-08-17 (epoch $START..$END) ===="
...
echo "[3] OpenAlex (3 queries, date-filtered 2026-08-11..2026-08-17)"
for pair in \
  "restaurant_kitchen_inventory|restaurant kitchen inventory" \
  "kitchen_display_system|kitchen display system" \
  "telegram_bot_restaurant|telegram bot restaurant"; do
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.openalex.org/works?search=$(printf '%s' "$q" | jq -sRr @uri)&per-page=20&filter=from_publication_date:2026-08-11,to_publication_date:2026-08-17"
  fetch "openalex" "$url" "openalex_${slug}.json"
done

echo "[4] GitHub Search Repositories (7 queries, created:2026-08-11..2026-08-17)"
for pair in \
  ...
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.github.com/search/repositories?q=$(printf '%s' "$q" | jq -sRr @uri)+created:2026-08-11..2026-08-17&sort=updated&order=desc&per_page=30"
...
ENVELOPE=/tmp/le31-daily-2026-08-17/index.json
