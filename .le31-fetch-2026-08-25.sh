#!/usr/bin/env bash
# LE31 daily research fetch — 2026-08-25
# 7-day window 2026-08-18..2026-08-25 UTC
# 26th consecutive daily-research pass.
set -u

RAW_DIR=/tmp/le31-daily-2026-08-25
LOG=$RAW_DIR/_fetch.log
INDEX=$RAW_DIR/index.json

UA="Mozilla/5.0"
START=1787616000   # 2026-08-18 00:00 UTC
END=1788071700     # 2026-08-25 06:35 UTC (exclusive upper bound)
...
for q in "restaurant%20kitchen%20inventory" "kitchen%20display%20system" "telegram%20bot%20restaurant"; do
  url="https://api.openalex.org/works?search=${q}&per_page=20&filter=from_publication_date:2026-08-18,to_publication_date:2026-08-25"
...
for q in "restaurant_python" "kitchen_display_python" "telegram_kitchen_python" "pos_python" \
  ...
  url="https://api.github.com/search/repositories?q=${q}+archived:false+created:2026-08-18..2026-08-25&per_page=30"
  fetch "$url" "ghsearch_${q}.json" "Authorization: token $GH_TOKEN"
done

# --- Direct repo star-velocity checks (watch list, 7 repos) ---
for repo in "longnick/small-pos-open-source" "satisfecho/pos" "helloman3/foodieshub" \
            "devnest-hq/restaurant-management-system" "SGrappelli/pronto" "jonalemndi2/ALdia" \
            "RafaelEmery/rafood-api"; do
