#!/bin/bash
# LE31 daily research fetcher — 2026-08-10
# Window: 2026-08-03 00:00 UTC .. 2026-08-10 23:59 UTC (last 7 days)
# Epoch: 1785715200 .. 1786406399
# Mirrors fetch_le31_2026-08-09.sh pattern.

set -u

UA='LE31Research/1.0 (research; contact: hermes-agent@nousresearch.local)'
RAW=/tmp/le31-daily-2026-08-10
START=1785715200
END=1786406399
INDEX=/tmp/le31-daily-2026-08-10/index.json.tmp
> "$INDEX"
echo '[' > "$INDEX"
...
for pair in \
  ...
  slug=${pair%%|*}; q=${pair##*|}
  url="https://api.openalex.org/works?search=$(printf '%s' "$q" | jq -sRr @uri)&filter=from_publication_date:2026-08-03,to_publication_date:2026-08-10&per_page=20"
...
sed -i '$ s/,$//' "$INDEX"
echo ']' >> "$INDEX"
mv "$INDEX" /tmp/le31-daily-2026-08-10/index.json
