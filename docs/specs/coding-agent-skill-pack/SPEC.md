...
- A separate `docs/coding-agent/` folder in this repo makes the pack shippable and `.gitignore`-able in the destination repo.

## Acceptance

- 7 SKILL.md files at `docs/coding-agent/skills/<name>/SKILL.md`.
- Each skill cites its primary source in the body and includes verification commands.
- Plus one `README.md` and one `checklist.md` in `docs/coding-agent/`.
- SPEC/PLAN/TASKS files at `docs/specs/coding-agent-skill-pack/`.
