...
2. Copy the entire `docs/coding-agent/` folder to its `skills/le31-pack/` (or your preferred path) inside the destination repo. Use the same nested `skills/<name>/SKILL.md` layout.
...
5. Verify the new skills are listed:
   ...
   ```
6. Drop a `docs/coding-agent/.gitignore` marker in the destination repo so the pack is ignored locally after copying:
   ```gitignore
   /skills/le31-pack/
   ```
