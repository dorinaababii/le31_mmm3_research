#!/usr/bin/env bash
# LE31 daily research fetcher — 2026-09-08
# Window: 2026-09-01 00:00 UTC .. 2026-09-08 06:30 UTC (7-day; epoch 1788134400..1788763800).
# Mirrors prior fetch_le31_*.sh patterns (see git log for fetch_le31_2026-09-06.sh).
#
# Raw response convention: this cron's write-safe root is /opt/data, so all raw
# fetches land under /opt/data/le31-daily-2026-09-08/ (not /tmp/... as the
...
# language:python, pushed:>2026-09-01), GitHub direct-repo GET (watchlist of
# 15), PyPI JSON (9 packages), GitHub releases.atom (6 repos), ECB
...
UA="Mozilla/5.0"
START=1788134400   # 2026-09-01T00:00:00Z
END=1788763800     # 2026-09-08T06:30:00Z
...
###############################
# 4. GitHub Search Repositories — 8 queries (added append-only+ledger)
###############################
declare -a QUERIES=(
  "fastapi+restaurant"
...
for Q in "${QUERIES[@]}"; do
  ...
  ENC=$(python3 -c "import urllib.parse,sys;print(urllib.parse.quote(sys.argv[1]))" "$Q")
  URL="https://api.github.com/search/repositories?q=${ENC}+language:python+pushed:>2026-09-01&sort=updated&order=desc&per_page=30"
...
###############################
# 5. GitHub Watchlist — 15 repos direct repo GET
###############################
REPOS=(
  ...
  "giltotherescue/slashbooks"
  "martymcenroe/silphe"
  "mrlonis/python-open-restaurant-fastapi"
)
for R in "${REPOS[@]}"; do
  SAFE="${R//\//__}"
