...
> `features/94-pronto-watch-v2.md` before touching any code. Do not
> paste chat excerpts back into the build.

## Frozen identifiers (do not rename)

- Feature ID: `94`
- Slug: `pronto-watch-v2`
- Contract file: `features/94-pronto-watch-v2.md`
...
**40★** (+1★/24h); 11 → **12 forks** (+1 fork/24h — FIRST positive
fork movement on this peer); new push **2026-08-21T19:07:53Z**
(~11.4h before today's fetch); TypeScript stack; WhatsApp + Telegram
reminders; self-hosted Docker; MIT license).

**Confidence:** **high** for the cross-section pattern (WhatsApp +
Telegram reminders is a direct cross-section to LE31's cook-bot
surface); **high** for the day-2 movement (1★/24h + 1 fork/24h + new
push); **low** for the build implication (charter §3.1 says
...
```
features/94-pronto-watch-v2.md              # NEW (this artifact)
specs/pronto-watch-v2-HANDOFF.md            # NEW (this file)
...
1. **Read back**
   `features/94-pronto-watch-v2.md` and confirm it matches the
   daily-research report's "pronto-watch-v2" pick description.
2. **Read back** the new row in `INDEX.md` "Active feature pipeline"
   table and confirm the date (2026-08-22), pick slug
   (`pronto-watch-v2`), feature path
   (`features/94-pronto-watch-v2.md`), and Linear sub-issue ID.
...
v1 bucket for the Pronto surface — Pronto's WhatsApp + Telegram
reminders are a v1-adjacent cook-Telegram-bot surface expansion
question, not a v2-only watch-list entry). Label `Feature`.

- Title: `Feature 94 — pronto-watch-v2`.
- Body: the contract from
  `features/94-pronto-watch-v2.md` (or a short summary + the file
  path).
...
Delete `features/94-pronto-watch-v2.md` and this HANDOFF.md. Remove
...
WhatsApp + Telegram reminders, (c) cafe/booking/CRM surface. The day-2
observation (+1★/24h + +1 fork/24h + new push 2026-08-21T19:07:53Z)
is the first concrete signal of community traction since the 2026-08-21
baseline. The pattern is direct and worth documenting. The
cross-section signals that the small-business self-hosted POS market
is converging on dual-channel bot reminders (Telegram + WhatsApp).
LE31's charter §3.1 currently says Telegram-only; the "expand to
WhatsApp" question is a charter-level decision that Pronto's existence
supports but does not yet require. The artifact is filed as a
watch-list entry with a clear re-evaluation trigger so the next
research pass knows what to look for.

## Carry-over history

This is the **day-2 observation** (2026-08-22); the day-1 baseline
is at `features/90-pronto-cafe-telegram-reminders-cross-section.md`
(2026-08-21). Combines:

- The cross-section pattern from `SGrappelli/pronto` (now ★40,
  pushed 2026-08-21T19:07:53Z, TypeScript, WhatsApp + Telegram
  reminders).
- The 23-pass observation that LE31's cook-bot surface is
