# 213 — hseshadr-avow-mit-signed-evidence-receipts-append-only-ledger-v2-ai-evidence-receipts HANDOFF
...
`features/213-hseshadr-avow-mit-signed-evidence-receipts-append-only-ledger-v2-ai-evidence-receipts.md` (defer artifact; **no code today**).

Bucket: **v2-AI signed-evidence-receipts (architecture-reference)**. Build verdict: **`defer`** (charter §3.1 + §3.4 + `le31-conventions` Feature gate).
...
**NONE today.** The defer artifact is documentation only. Files to touch *if* the future v2-AI trigger condition fires (first v2-AI PR that introduces an AI-assisted workflow with evidence-binding + audit-trail):

- `app/models/signed_evidence_receipts.py` — possibly add a `signed_evidence_receipts` SQLModel table (receipt_id, actor_user_id, assertion, signature_algorithm, signature_value, public_key_id, recorded_at, prev_receipt_hash).
- `app/models/signing_keys.py` — possibly add a `signing_keys` SQLModel table (key_id, algorithm, public_key_bytes, owner_user_id, created_at, revoked_at).
- `app/models/audit_log.py` — possibly add a nullable `signature_id` FK column to `audit_logs` (existing rows remain signature-less; new AI-assisted workflow entries would carry signatures).
- `app/models/signature_verification_log.py` — possibly add a `signature_verification_log` SQLModel table (verification_id, receipt_id, verified_at, verification_result, error_message).
- `app/routes/verify_receipt.py` — possibly add a `verify_receipt(receipt_id) -> bool` endpoint that re-verifies a single receipt on demand.
- `app/bot/receipt_show.py` — possibly add a Telegram-bot command for the owner to view a *signed-evidence-receipt* (e.g., `/receipt show <receipt_id>`).
- `app/bot/receipt_verify.py` — possibly add a Telegram-bot command for the owner to verify a receipt's signature (e.g., `/receipt verify <receipt_id>`).
...
- [x] `features/213-hseshadr-avow-mit-signed-evidence-receipts-append-only-ledger-v2-ai-evidence-receipts.md` exists and is read back by the parent.
- [x] `specs/213-hseshadr-avow-mit-signed-evidence-receipts-append-only-ledger-v2-ai-evidence-receipts-HANDOFF.md` (this file) exists and is read back by the parent.
- [x] The GitHub repo description is quoted verbatim: *"Signed evidence receipts and an append-only ledger."*
- [x] The MIT permissive license is documented.
- [x] The *signed evidence receipts + append-only ledger* double-primitive set is documented.
...
- [ ] The *signed evidence receipts + append-only ledger* double-primitive set is evaluated against the PR's changes: does the change preserve *append-only posture*? Does the change add *signed-evidence-receipts*? Does the change add *signature-verification*?
...
- Delete: revert the PR; remove the new `signed_evidence_receipts` table; remove the new `signing_keys` table; remove the new `signature_verification_log` table; remove the new `signature_id` FK column from `audit_logs`; restore the original `audit_logs` schema.
- Migration cost: depends on the v2-AI change; the *signed evidence receipts + append-only ledger* primitive set's design implies *additive schema* (the new `signed_evidence_receipts` + `signing_keys` tables are additive on top of the existing append-only log).
...
|---|---|
| Feature ID | 213 |
| Slug | `hseshadr-avow-mit-signed-evidence-receipts-append-only-ledger-v2-ai-evidence-receipts` |
| Bucket | v2-AI signed-evidence-receipts (architecture-reference, parking-lot defer) |
...
| Linear sub-issue | linear: blocked (workspace plan limit); intended title `Feature 213 — hseshadr-avow-mit-signed-evidence-receipts-append-only-ledger-v2-ai-evidence-receipts` |
| Status today | defer (parking-lot) |
| No code today | yes |
| Verifiability today | feature file + HANDOFF file + INDEX.md row + parent research report file |
| Trigger condition (future v2-AI) | first v2-AI PR that introduces an AI-assisted workflow with evidence-binding + audit-trail |
| Disable path | delete `features/213-...md` + `specs/213-...-HANDOFF.md` |
...
| GitHub repo | `hseshadr/avow` (0star/0forks, Python, 443 KB, pushed 2026-09-16T16:10:13Z) |
| Topics (verbatim) | none (0 topics) |
| Description (verbatim) | *"Signed evidence receipts and an append-only ledger."* |
| LE31-shape-score | 2 (description-only signals) |
| Companion artifacts | features 92, 126, 127, 128, 133, 134, 137, 145, 152, 167 (provtrail hash-chained ledger), 169, 175, 183, 187, 188, 189, 190, 192, 196, 197 (rajo69/ledgerkb — repo now 404, vocabulary-only), 198, 199, 203 (shawn-durrani/membro v2-AI control-plane), 204 (docentesIA/dagwell v2-AI orchestration), 205 (Mark007-R/Restaurant-Intelligence-Platform v1 restaurant-vertical reference), 212 (sister-pick A), 214 (sister-pick C) (ripgrep-verified distinct) |
