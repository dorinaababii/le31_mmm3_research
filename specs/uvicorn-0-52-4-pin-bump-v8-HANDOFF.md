...
> `features/99-uvicorn-0-52-4-pin-bump-v8.md` before touching any
...
- Contract file: `features/99-uvicorn-0-52-4-pin-bump-v8.md`
- Bucket: **v2 utility (stack pin-bump)** — build candidate defer
- Linear parent: `HMM-129` (Research 2026-08-23 — daily, created in this cron)
...
**observed** (verified in-window across the 24-pass series via two
...
stabilized at 0.52.4 with no 0.52.5 release in 120h).
...
the burst has stabilized at 0.52.4 after 120h with no 0.52.5 release).
...
```
features/99-uvicorn-0-52-4-pin-bump-v8.md    # NEW (this artifact)
specs/uvicorn-0-52-4-pin-bump-v8-HANDOFF.md  # NEW (this file)
...
1. **Read back** `features/99-uvicorn-0-52-4-pin-bump-v8.md` and
   confirm it matches the daily-research report's
   "uvicorn-0-52-4-pin-bump-v8" pick description.
2. **Read back** the new row in `INDEX.md` "Active feature pipeline"
   table and confirm the date (2026-08-23), pick slug
   (`uvicorn-0-52-4-pin-bump-v8`), feature path
   (`features/99-uvicorn-0-52-4-pin-bump-v8.md`), and Linear
   sub-issue ID.
...
6. **On a future daily-research pass**: re-query the GitHub
   ...
   0.52.5. **Re-check on 2026-08-24 for new in-window uvicorn
   releases.**
...
- Title: `Feature 99 — uvicorn-0-52-4-pin-bump-v8`.
- Body: the contract from
  `features/99-uvicorn-0-52-4-pin-bump-v8.md` (or a short summary +
  the file path).
...
Delete `features/99-uvicorn-0-52-4-pin-bump-v8.md` and this HANDOFF.md.
...
releases in 7 days suggests urgency but none is a CVE/security fix;
the burst has now stabilized at 0.52.4 after 120h with no 0.52.5
...
This is the **8th consecutive day** the uvicorn pin-bump has surfaced
...
- 2026-08-22 → `features/93-uvicorn-0-52-4-pin-bump-v7.md` (target 0.52.4, v7)
- 2026-08-23 → `features/99-uvicorn-0-52-4-pin-bump-v8.md` (target 0.52.4, v8, **this artifact**)