# uvicorn-0-52-4-pin-bump-v6 — HANDOFF

> **Slice for the coding agent.** Read this *and*
> `features/87-uvicorn-0-52-4-pin-bump-v6.md` before touching any
> code. Do not paste chat excerpts back into the build.

## Frozen identifiers (do not rename)

- Feature ID: `87`
- Slug: `uvicorn-0-52-4-pin-bump-v6`
- Contract file: `features/87-uvicorn-0-52-4-pin-bump-v6.md`
...
**observed** (verified in-window across the 22-pass series via two
...
stabilized at 0.52.4 with no 0.52.5 release in 72h).
...
the burst has stabilized at 0.52.4 after 72h with no 0.52.5 release).
...
```
features/87-uvicorn-0-52-4-pin-bump-v6.md   # NEW (this artifact)
specs/uvicorn-0-52-4-pin-bump-v6-HANDOFF.md # NEW (this file)
...
1. **Read back** `features/87-uvicorn-0-52-4-pin-bump-v6.md` and
   confirm it matches the daily-research report's
   "uvicorn-0-52-4-pin-bump-v6" pick description.
2. **Read back** the new row in `INDEX.md` "Active feature pipeline"
   table and confirm the date (2026-08-21), pick slug
   (`uvicorn-0-52-4-pin-bump-v6`), feature path
   (`features/87-uvicorn-0-52-4-pin-bump-v6.md`), and Linear
   sub-issue ID.
3. **Read back** the dependency manifest and confirm the pin is now
   ...
   `uvicorn == 0.52.4` if the team already bumped per feature 80/82).
...
6. **On a future daily-research pass**: re-query the GitHub
   ...
   0.52.5. **Re-check on 2026-08-22 for new in-window uvicorn
   releases.**
...
- Title: `Feature 87 — uvicorn-0-52-4-pin-bump-v6`.
- Body: the contract from
  `features/87-uvicorn-0-52-4-pin-bump-v6.md` (or a short summary +
  the file path).
...
Delete `features/87-uvicorn-0-52-4-pin-bump-v6.md` and this HANDOFF.md.
...
releases in 7 days suggests urgency but none is a CVE/security fix;
the burst has now stabilized at 0.52.4 after 72h with no 0.52.5
...
This is the **6th consecutive day** the uvicorn pin-bump has surfaced
...
- 2026-08-21 → `features/87-uvicorn-0-52-4-pin-bump-v6.md` (target 0.52.4, v6 — this artifact)

Consistent deferral pattern with the same highest-patch-version target
(0.52.4) for 3 consecutive days — the burst has stabilized.
